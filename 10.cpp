...
const char from_kind = 'F';
const string from_key = "from";
bool from_x = false;
ifstream ifs;
const char to_kind = 'T';
...
//reading characters from cin and drawing up a Token
{
    ...
    default:
        if (isalpha(ch))
        {
            ...
            if (s == from_key)
                return Token{from_kind, s}; //input data from file
            if (s == to_key)
                return Token{to_kind, s}; //output data to file
            return Token{name, s};
        }
        error("Bad Token");
    }
...
void from_file()
{
    ...
    if (is_eof())
        cout << "Input file is empty! Try again!" << endl;
    else
    {
        t = ts.get();
        if (t.kind == to_kind)
